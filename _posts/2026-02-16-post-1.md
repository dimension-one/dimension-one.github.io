...
<div class="tldr">
  <strong>TL;DR.</strong>
  If a generalization bound does not depend on the data, 
  it must hold for all datasets. 
  But there exist datasets with random labels 
  where training error is zero and population error is as bad as random guessing.
  Therefore the bound must allow error as large as random guessing 
  for every dataset, and is uninformative. 
</div>

This post illustrates this point in one dimension. 
...
You cannot observe it directly. It is the **population error**, 
the expected error rate over future independent attempts at baking a cake with these fixed settings. 

Generalization is about understanding when and why population error stays close to train error.

{::nomarkdown}
$$
{\small
\begin{aligned}
\text{generalization gap} =& \text{population error} - \text{train error}\\
=& \text{expected error on independent future inputs} \\
& - \text{error on inputs used to adjust the settings}
\end{aligned}
}
$$
{:/nomarkdown}
...
{::nomarkdown}
$$
\text{population error} \;\le\; \text{train error} + \text{complexity(model, data, optimizer)}
$$
{:/nomarkdown}

The goal here is not to certify population error.
In practice, we use held-out test sets for that. 
(If data is i.i.d., then concentration bounds guarantee that empirical test error  {::nomarkdown}$\approx${:/nomarkdown} population error with high probability).

Generalization bounds aim at something different: 
they try to explain *why* generalization happens.
They are not meant to replace population error estimation, 
...
In machine learning textbooks, complexity is first introduced as a property of the model class:
- number of parameters  
- architectural choices (linear versus nonlinear, convolutional versus fully connected, etc.)
...
so the generalization gap is simply the population error. 

What should the population error be?
...
So the best possible population error we can observe, called Bayes error, is:
...
n_train, n_test = 32, 1000 # n_test large enough to sample [-1,1] with high resolution and get accurate population error estimate
...
## what happens: model fits everything, population error depends on data

We obtain the next plot.

![train/population error vs label corruption]({{ "/assets/images/post-1/data_dependence_1d_results.png" | relative_url }})


Two observations:

- train error is near 0 for all {::nomarkdown}$p${:/nomarkdown}: the model fits everything, even pure noise;
- population error tracks the Bayes error {::nomarkdown}$p/2${:/nomarkdown}. 

Same model, same optimizer, but different data (here, data with different corruption levels) leads to different generalization.
...
Suppose the complexity measure does **not** depend on the labels. 
Then it must upper bound the generalization gap for all possible label assignments, including completely corrupted ones. 
But in that case, no model can beat random guessing. 
Because the complexity term does not depend on the labels, once it is forced to be at least 1/2 in the fully corrupted case, it is at least 1/2 for every dataset. 
...
$$
\text{gap} =
\text{population error} - \text{train error} \approx p/2 - 0 = p/2
$$
{:/nomarkdown}
...
In other words, it must work even when the labels are completely random. 
That implies:
...
This includes complexity measures such as VC dimension and Rademacher complexity, which quantify the expressive power of a hypothesis class, sometimes in a distribution-dependent way through the inputs, but *independently of the labels being fitted and the optimization process*. In the present setting, the class is rich enough to fit arbitrary label assignments. These quantities are therefore necessarily large and yield uninformative bounds. A smaller, data-dependent subclass could in principle have lower complexity, but the conclusion remains that the input-label distribution must enter the explanation.
...
@misc{gonon2026data-dependence-generalization,
  ...
  year   = {2026},
  url    = {https://dimension-one.github.io/blog/2026-02-16-data-dependence-generalization/}
}
```